...
class Game:
    ...
    def players_score(players,rounds,games_in_round):
        for i in range(games_in_round):
            ...
            if rounds.pairing_players[i][0].elo - rounds.pairing_players[i][1].elo > 130:
                cast_point_white = 1
                cast_point_black = 0
            if 0 <= rounds.pairing_players[i][0].elo - rounds.pairing_players[i][1].elo <= 65:
                cast_point_white = cast_point_black = 0.5
            if rounds.pairing_players[i][1].elo - rounds.pairing_players[i][0].elo > 130:
                cast_point_white = 0
                cast_point_black = 1
    ...
    def sorted_paring_players(rounds,games_in_round):
        ...
        for i in range(games_in_round):
            sum_points = rounds.pairing_players[i][0].score_after_last_game + rounds.pairing_players[i][1].score_after_last_game
            score_list.append(sum_points)
        i = 0     
        while len(score_list) > 0:
            sum_points = pairing_list_temp.pairing_players[i][0].score_after_last_game + pairing_list_temp.pairing_players[i][1].score_after_last_game
            if sum_points == max(score_list):
                score_list.remove(max(score_list))
                list_best_in_top.append([pairing_list_temp.pairing_players[i][0],pairing_list_temp.pairing_players[i][1]])
