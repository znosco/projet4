...
for i in range(rounds_in_tournament):
    ...
    Controller.manage_odd_nomber_players(
        players, rounds[i], pn, round_number, games_in_round
    )
    Display.pairing_before_round(players, games[i], console, round_number)
    Game.players_score(players, rounds[i], games_in_round)
    Controller.manage_odd_nomber_players(
        players, rounds[i], pn, round_number, games_in_round, played=True
    )
    Display.print_score_after_last_round(
        players, games[i], rounds[i], console, round_number, pn
    )
    round_number += 1
