    ...
    def pairing(self, players, games_in_round, round_number, pn):
        ...
        def play_game(self, players, games_in_round):
            ...
            for i in range(games_in_round):
                self.pairing_players.append([players[i], players[i + games_in_round]])
            ...
            def seek(self, players, games_in_round):
                ...
                while match < games_in_round:
                    ...
                    for j in range(pn - 1):
                        ...
                        for i in range(games_in_round):
                            delta = abs(
                                players[i].score_after_last_game
                                - players[i + games_in_round].score_after_last_game
                            )
                            no_played_together = can_you_play_with_me(
                                players, number_game=0
                            )
                            if delta <= 0 + n and no_played_together:
                                count += 1
                        if match < count:
                            match = count
