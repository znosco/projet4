class Controller:
    ...
    def manage_odd_nomber_players(
        players, rounds, pn, round_number, games_in_round, played=False
    ):
        if pn % 2 == 0:
            pass
        else:
            if round_number == 1:
                if played:
                    print(
                        f"le joueur {players[pn-1].name_elo} ne"
                        " joue pas cette ronde et gagne 1 point"
                    )
                ...
                if not played:
                    ...
                    while not find:
                        ...
                        for j in range(2):
                            if (
                                rounds.pairing_players[i][j].name
                                in Controller.move_player.opponent
                            ):
                                print()
                                Controller.no_play = rounds.pairing_players[i][
                                    j
                                ].name_elo
                                Controller.move_player.opponent.remove(
                                    rounds.pairing_players[i][j].name
                                )
                                rounds.pairing_players[i][j].score_after_last_game += 1
                                rounds.pairing_players[i][j].score_game.append(1)
                                rounds.pairing_players[i].insert(
                                    j, Controller.move_player
                                )
                ...
                if played:
                    print(
                        f"\n le joueur {Controller.no_play} ne"
                        " joue pas cette ronde et gagne 1 point \n"
                    )
    ...
    def manage_user_main():
        ...
        while not select_pn:
            pn = input(
                "veuillez entrer le nombre de joueurs (un nombre entre 4 et 100000)"
            )
            try:
                pn = int(pn)
                if pn >= 4:
        ...
        while not select_rounds_in_tournament:
            rounds_in_tournament = input(
                "veuillez entrer le nombre de rondes(tour de jeu)"
            )
            try:
    ...
    def verify_data(tournament_table, players_table, games_table, results_table):
        choice = input(
            "voulez-vous inspecter les données sauvegardées? \
