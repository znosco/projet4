...
class Display:        
    ...
    def print_score_after_last_round(players, games, rounds, console, round_number, pn):
        players_temp = sorted(players, 
                              key=lambda x: x.score_after_last_game, reverse=True)
        print()
        print (f'----------résultat de la partie et scores des joueurs après la ronde: {round_number} -------------------------------')
        print()
        ...
        for i in range(pn):
            ...
            if i==0:
                ...
                if players_temp[i-1].score_after_last_game == players_temp[i].score_after_last_game:
                    same_score = True 
                if same_score == False:
                    j=i+1 
